...
- 自己标注一部分数据集之后到`roboflow`去增加规范数据集格式以及增加数量
  - ![](./picture/roboflow_1.png)
  - 选择`Projects`然后点击右上角的`New Project`
  - ![](./picture/roboflow_2.png)
  - 选择`Object Detection`然后填入`Project Name`，完成后会自动帮你将图片以及标注信息分类好
  - 完成后点击`Versions`
  - ![](./picture/roboflow_3.png)
  - `Resize`选择`640 x 640`
  - 在`Augmentation`中选择`Flip`和`Rotation`（默认的正负15°），接着正常生成就行
  - 生成完成后直接`Download`
    - `Download zip to computer`是下载到本地
    - `Show download code`是下载到服务器

### 服务器训练数据集

https://colab.research.google.com/github/Yogender-Singh/Notebooks/blob/main/yolo-series/tutorial.ipynb#scrollTo=1NcFxRcFdJ_O

- 先到页面右上角点击连接GPU

- 到终端粘贴`roboflow`的命令行
  - 运行`Setup`中的命令行（配置服务器环境）
  - ![](./picture/Colab_1.png)
  - 到`Train`中复制左侧栏目中的`data.yaml`路径粘贴到命令行中`data`的位置，然后选择训练轮次（`epoch`），开始运行程序
  - 训练出来的损失率越接近1越好，`mAP>=0.7`和`mAP0.5~mAP0.95>=0.5`
- 下载整个文件夹
  - 在终端中输入`zip -r download.zip /content/your_folder`
  - 然后像下载文件一样下载下来
...
- 选择`GroundingDINO(SwinB-QInt8)`，在框内输入文本内容就能自动识别
  - 多个文本信息就使用`.`来隔开，示例：`crack.pothole.`
- 使用自己模型就选择`..load_custom_model`，然后选择自己的`classes.txt`
